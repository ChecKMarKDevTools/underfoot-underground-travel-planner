...
- Find similar cached results using vector similarity
  */
  const findSimilarCachedResults = async (
  query,
  location,
  similarityThreshold = VECTOR_CONFIG.similarityThreshold,
  ) => {
  try {
...
- Store search results with vector embeddings in semantic cache
  */
  const setSemanticCachedResults = async (
  query,
  location,
  results,
  ttlMinutes = VECTOR_CONFIG.cacheTtlMinutes,
  ) => {
  try {
  const [queryEmbedding, locationEmbedding] = await Promise.all([
  generateEmbedding(query),
...
- Find relevant underground keywords for query enhancement
  */
  ...
  const { data, error } = await supabase.rpc('find_underground_keywords', {
  query_embedding: queryEmbedding,
  similarity_threshold: similarityThreshold,
  result_limit: 10,
  });
