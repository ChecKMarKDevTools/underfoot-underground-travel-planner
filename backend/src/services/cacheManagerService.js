  ...
  try {
    ...
    if (memoryCache.has(memoryKey)) {
      ...
      if (cached.expires > Date.now()) {
        cacheMetrics.hits++;
        logPerformance('memory_hit', Date.now() - startTime);
        return {
          cached: true,
          source: 'memory',
          results: cached.data,
...
const calculateAdaptiveTtl = async (query, location) => {
  try {
    if (!supabaseAdmin) {
      return CACHE_CONFIG.defaultTtlMinutes;
    }
    ...
    const { data: popularityData } = await supabaseAdmin
      ...
      .gte('access_count', CACHE_CONFIG.popularityThreshold)
      .limit(1);

    if (popularityData && popularityData.length > 0) {
      return CACHE_CONFIG.popularQueryTtlMinutes;
    }
...
const warmRelatedQueries = async (originalQuery, originalLocation) => {
  ...
  try {
    ...
    const { data: similarQueries } = await supabaseAdmin
      .from('semantic_cache')
      .select('original_query, location, access_count')
      .neq('original_query', originalQuery)
      .gte('access_count', 2)
      .order('access_count', { ascending: false })
      .limit(CACHE_CONFIG.warmupBatchSize);
