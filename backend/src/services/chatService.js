...
export default class ChatService {
  ...
  async handleChat(body) {
    const { message = '', limit, force = false } = body || {};
    ...
    if (limit !== undefined) {
      ...
      if (!Number.isInteger(numLimit) || numLimit < 1 || numLimit > 20) {
        const err = new Error('Limit must be an integer between 1 and 20.');
        err.status = 400;
        throw err;
      }
    ...
    const tiers = [
      userRadius,
      Math.min(userRadius * 2, this.maxRadius),
      Math.min(userRadius * 4, this.maxRadius),
    ];
    ...
    const payload = {
      ...
      debug: {
        ...
        radiusUsed:
          tiers.findLast(
            (_) => gathered.core.length + gathered.stretch.length + gathered.nearby.length > 0,
          ) || userRadius,
  ...
  async composeReply(parsed, primary, nearby) {
    if (!config.OPENAI_API_KEY) {
      ...
      const n = nearby.length
        ...
        : '';
      const radiusNote =
        parsed.radius !== this.defaultRadius
