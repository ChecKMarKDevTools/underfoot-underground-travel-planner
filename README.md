...
```
/underfoot/chat?message=Your+prompt&stream=true
```
...
```js
const es = new EventSource('/underfoot/chat?stream=true&message=' + encodeURIComponent('Hello'));
...
Fallback: You can continue using `POST /underfoot/chat` with `{ message }` for non-streaming clients.
