...
const createMarkerSVG = (color: string, isSelected: boolean) => {
  ...
  return `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">   <circle cx="12" cy="12" r="10" fill="${color}" stroke="#ffffff" stroke-width="2"/>   <circle cx="12" cy="12" r="4" fill="#ffffff"/> </svg>`;
};
...
export function GoogleMapView({
  ...
  const initMap = useCallback(() => {
    if (!mapRef.current || !isLoaded || isInitialized) {
      return;
    }
