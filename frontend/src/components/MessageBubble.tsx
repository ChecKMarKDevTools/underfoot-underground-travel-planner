...
\> <div className="flex items-start mb-2 w-full"> <span className="font-mono text-xs opacity-75 shrink-0 mr-auto">
{isUser ? 'You' : 'Stonewalker'} </span> <span className="font-mono text-xs opacity-75 shrink-0 ml-3">
{formatTimestamp(message.timestamp)} </span> </div> <p className="whitespace-pre-wrap leading-relaxed">{message.content}</p> </div>
...
\>
{place.imageUrl && ( <div className="w-full h-32 mb-3 rounded-lg overflow-hidden bg-void-200 dark:bg-void-700">
\<img
src={place.imageUrl}
alt={place.name}
