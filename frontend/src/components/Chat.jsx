...
export default function Chat({ onDebug }) {
  const [messages, setMessages] = useState([
    { from: 'bot', text: 'I’m Underfoot. Tell me where & when (e.g., ‘Pikeville KY next week for 3 days, outdoors’). I’ll find what the big sites missed.' }
  ])
      ...
      <div ref={threadRef} className='glass p-3 h-[65vh] overflow-auto' aria-live='polite'>
        <div className='flex flex-col gap-2'>
          {messages.map((m, i) => (
            <div
              key={i}
              className={`max-w-[85%] px-3 py-2 rounded-xl border ${m.from === 'user'
