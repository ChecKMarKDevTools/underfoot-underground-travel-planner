...
{JSON.stringify({
  ...
  requestId: data?.requestId
          ...
          <section className='glass p-3'>
            ...
            <pre className='mt-2 text-xs overflow-x-auto whitespace-pre-wrap'>{JSON.stringify(data?.raw ?? {}, null, 2)}</pre>
          </section>

          <section className='glass p-3'>
            <h3 className='text-base font-bold'>Filtered (truncated)</h3>
            <pre className='mt-2 text-xs overflow-x-auto whitespace-pre-wrap'>{JSON.stringify(data?.filtered ?? {}, null, 2)}</pre>
          </section>
        </div>
      </div>
    </div>
